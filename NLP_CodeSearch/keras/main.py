...
class CodeSearcher:
    ...
    def train(self, model):
        ...
        f1=open('/data/dengzhongyang/NLP_CodeSearch/keras/results/training_results.txt','a',encoding='utf-8',errors='ignore')
        for i in range(self.train_params['reload']+1, nb_epoch):
            ...
            if valid_every is not None and i % valid_every == 0:
                sr1,mrr = self.valid(model,10)
                if mrr>BestMRR:
                    BestMRR=mrr
                    BestEpoch=i
                print("BestMRR={},Epoch={}".format(BestMRR,BestEpoch))
                f1.write('epoch={},SR1={}, MRR={}'.format(i,sr1,mrr)+'\n')  
